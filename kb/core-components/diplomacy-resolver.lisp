...
(defun get-adjacent-regions(region)
  (let (
        (all-regions (list-instances {diplomacy region}))
        (children (list)))
        
        (loop for element in all-regions
          do(if (statement-true? element {adjacent to} region)
              (setq children (append children (list element)))
              )          
          )
    (return-from get-adjacent-regions children)  
    )
  )

(defun in-list(item list-to-check)
  (loop for element in list-to-check
  do (if (equal-diplomacy-elements element item)
...
(defun bfs (start end)
  (if (equal-diplomacy-elements start end) (return-from bfs (list start end)))
  (let ((i 0) 
        (frontier (list start))
        (visited (list start))
        (path (list start)))
    (loop while (< i (length frontier))
      do(if (equal-diplomacy-elements (nth i frontier) end) 
            (setf path (append end path))) 
          (setq i (+ i 1))
...
(defun bfs (explored frontier end i path)
  (let ((popped (nth i frontier))
        )
    (cond ((equal-diplomacy-elements end popped)
           (setq frontier (append frontier (get-adjacent-regions popped)))
...
(setf (get '{PAR} 'adjacent) '({LON}))




 (defun bfs (goal start number-of-nodes)
   (labels ((next-path (path)
              (if (= (length path) number-of-nodes) (return-from next-path nil))
              (mapcar (lambda (head) (cons head path))
                      (remove-if (lambda (adj) (in-list adj path))
                                 (get-adjacent-regions (car path) ))))
            (iter (plist)
                  (if (null plist)
                      nil
                      (destructuring-bind (path &rest q) plist
                        (if (equal-diplomacy-elements (car path) goal)
                            (cons (reverse path) (iter q))
                            (iter (append q (next-path path))))))))
     (iter `((,start))
     )))
...
(defun pathsFromXToY (start end)
  (let explored (vector))
  (let frontier (vector start))
  (bfs explored frontier end 0 (vector))
  )



(defun player (context)
